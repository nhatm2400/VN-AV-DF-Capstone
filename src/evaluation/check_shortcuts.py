...
def evaluate_feature_rows(rows, threshold=0.65, seed=42):
    ...
    if any(not row.get("group_id") for row in rows):
        raise ValueError("Metadata diagnostic requires speaker/source connected group_id")
    groups = np.array([str(row["group_id"]) for row in rows])
    ...
    for group in unique_groups:
        group_rows = [row for row in rows if str(row["group_id"]) == group]
        if {int(row["label"]) for row in group_rows} != {0, 1}:
            raise ValueError(f"Group {group} needs real and fake examples")
    ...
    return {
        ...
        "cv": "GroupKFold(n_splits=5), group=connected speaker/source group_id",
    ...
    for label, side in ((0, real_rows), (1, fake_rows)):
        for row in side:
            if row.get("split") == "test":
                raise ValueError("Run development diagnostics on train/val, not final test")
            source = row.get("source_clip") or row.get("parent_clip_id") or row["clip_id"]
            method = "real" if label == 0 else row.get("generator") or row.get("method", "")
...
def write_predictions_atomic(path, rows, predictions):
    fields = ["clip_id", "source_clip", "group_id", "method", "label", *FEATURE_NAMES,
