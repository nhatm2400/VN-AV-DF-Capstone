...
- **Output:** `data/02_curate/tier1_scored_all.csv` (manifest + face stats),
  `data/02_curate/embeddings_all.npy` (N×512, đã L2-norm)
...
- **Input:** `data/02_curate/tier1_scored_all.csv` + `--syncnet_dir <repo>`
- **Output:** `data/02_curate/calibrate_sync_results.csv` (calibrate) hoặc CSV có thêm
...
để chọn ngưỡng.
- **Input:** `data/02_curate/tier1_scored_all.csv` + `data/02_curate/embeddings_all.npy`
- **Output:** `data/02_curate/all_clean.csv` (tập sạch, có `speaker_id`),
  `data/02_curate/all_clean_rejects.csv` (clip bị gate loại)
...
- **Input:** `data/02_curate/tier1_scored_all.csv` (+ `all_clean.csv`)
