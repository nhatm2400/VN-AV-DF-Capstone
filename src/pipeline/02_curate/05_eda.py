...
LOCAL (mặc định — chạy KHÔNG cần tham số, từ thư mục gốc dự án):
  python 05_eda.py
  -> đọc data/curate/tier1_scored_all.csv + all_clean.csv, xuất data/curate/eda_figs/
...
def main():
    ...
    ap.add_argument("--clean_csv", default="data/curate/all_clean.csv",
                    help="output 04_curate: tập sạch có speaker_id (tùy chọn)")
    ap.add_argument("--out_dir", default="data/curate/eda_figs")
    args = ap.parse_args()
