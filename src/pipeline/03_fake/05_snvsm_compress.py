...
def compress(in_path, out_path, crf, preset):
    """Re-encode video libx264 @CRF (audio copy, fallback aac). Cùng cấu hình cho real+fake."""
    def run(acodec):
...
def main():
    ...
    for i, r in enumerate(rows):
        ...
        chosen = crfs if args.mode == "all" else [random.choice(crfs)]
        for crf in chosen:
            new_id = f"{src_id}_crf{crf}"
            out_path = os.path.abspath(os.path.join(args.out_dir, new_id + ".mp4"))
            if compress(src_path, out_path, crf, args.preset):
                row = dict(r)
    ...
    mf.close()
    print(f"\nXong. Clip nén: {made} | bỏ qua: {skipped} | lỗi: {failed}")
