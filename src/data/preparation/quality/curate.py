...
LOCAL (mặc định — chạy KHÔNG cần tham số, từ thư mục gốc dự án):
  python src/data/preparation/quality/curate.py --calibrate   # 1) xem phân bố, chọn ngưỡng
  python src/data/preparation/quality/curate.py               # 2) export -> data/manifests/dataset_v1/curated.csv
  -> tự đọc cache/quality/dataset_v1/tier1_scored_all.csv + embeddings_all.npy

Ví dụ tùy chỉnh / KAGGLE:
  # 1) Xem phân bố + số cụm ở vài ngưỡng (không xuất gì)
  python src/data/preparation/quality/curate.py --scored_csv tier1_scored_tier1.csv --emb embeddings_tier1.npy --calibrate

  # 2) Chốt ngưỡng rồi xuất
  python src/data/preparation/quality/curate.py --scored_csv ... --emb ... \\
...
def main():
    ...
    ap.add_argument("--scored_csv", default="cache/quality/dataset_v1/tier1_scored_all.csv",
                    help="output của 02_scoring/01_face_quality.py (mặc định local)")
    ap.add_argument("--emb", default="cache/quality/dataset_v1/embeddings_all.npy")
    ...
    ap.add_argument("--out", default="data/manifests/dataset_v1/curated.csv")
