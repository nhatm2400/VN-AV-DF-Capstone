...
def main():
    ...
    parser.add_argument("--manifest", default="data/manifests/dataset_v1/clips.csv")
    parser.add_argument("--scores", required=True)
    parser.add_argument("--timeline", required=True)
    parser.add_argument("--out", default="data/manifests/dataset_v1/calibration/active_speaker_calibration.csv")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--per_group", type=int, default=150)
    parser.add_argument("--tune_per_group", type=int, default=100)
    ...
    if not tiers or not 0 < args.tune_per_group < args.per_group:
        raise ValueError("Need source groups and 0 < tune_per_group < per_group")
    ...
    for tier in tiers:
        ...
        chosen = choose_unique_sources(tier_rows, args.per_group, args.seed)
        ...
        chosen["calibration_split"] = ["tune"] * args.tune_per_group + ["locked_validation"] * (args.per_group - args.tune_per_group)
        selected.append(chosen)
        globally_used_sources.update(chosen["source_video"].astype(str))
    output = pd.concat(selected, ignore_index=True)
    if len(output) != args.per_group * len(tiers) or output["source_video"].duplicated().any():
