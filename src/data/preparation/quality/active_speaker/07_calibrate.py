...
def main():
    ...
    if (manifest.empty or manifest["clip_id"].isna().any()
            or manifest["clip_id"].duplicated().any()):
        raise ValueError("Calibration manifest must contain non-empty unique clips")
    ...
    if len(labels) != len(manifest):
        raise ValueError("Review labels do not cover the calibration manifest")
    ...
    if tune.empty or locked.empty:
        raise ValueError("Calibration requires non-empty tune and locked validation")
