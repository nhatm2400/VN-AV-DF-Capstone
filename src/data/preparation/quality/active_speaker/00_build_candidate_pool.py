...
def select_candidate_pool(frame: pd.DataFrame, per_tier: int, seed: int) -> pd.DataFrame:
    ...
    if not tiers or any(not tier.strip() for tier in tiers):
        raise ValueError(f"Invalid source groups: {tiers}")
    ...
    if len(output) != per_tier * len(tiers):
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest", default="data/manifests/dataset_v1/clips.csv"
    )
    parser.add_argument(
        "--out", default="data/manifests/dataset_v1/calibration/active_speaker_candidates.csv"
    )
