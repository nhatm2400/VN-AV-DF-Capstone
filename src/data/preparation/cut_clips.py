"""Reusable VAD/face/scene clip cutter with decode/encode fallback and immutable batches.
The tier field is a source-group tag, not a license. Configure explicit input paths.
CPU/libx264 is the portable default; GPU processing requires a tested environment."""
...
class CutConfig:
    ...
    dataset_dir: str
    input_csv: str
    output_root: str = "data/real/dataset_v1"
    run_id: str = ""
    start_index: int = 0
    end_index: int = 0
    expected_input_count: int = 0
    num_workers: int = 4
    face_model: str = "weights/face_detector/yolov8n-face.pt"
    ...
    def validate(self):
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*", self.tier):
        ...
        if self.expected_input_count <= 0:
            raise ValueError("expected_input_count phải khớp danh sách video đã chọn")
