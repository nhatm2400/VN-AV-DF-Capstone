...
def main():
    ...
    for path in result_paths:
        for row in read_csv(path):
            ...
            reasons_in_row = [r.strip() for r in row.get("reason", "").split(",") if r.strip()]
            if decision == "reject" and (not reasons_in_row or not all(r in REASONS for r in reasons_in_row)):
            ...
            if decision == "reject" and intervals and len(reasons_in_row) == 1 and row.get("reason", "") != longest_reason(intervals):
    ...
    for cid in manifest_by_id:
        ...
        reject_match = interval_match if all(r.get("rubric_version") == "v3" for r in rows) else (
            len({", ".join(sorted(x.strip() for x in (r.get("reason", "") or "").split(",") if x.strip())) for r in rows}) == 1)
