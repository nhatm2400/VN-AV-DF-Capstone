...
CORE_PATH = ROOT / "src/data/preparation/cut_clips.py"
SPEC = importlib.util.spec_from_file_location("cut_clips_core_test", CORE_PATH)
CORE = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = CORE
SPEC.loader.exec_module(CORE)
PREP_PATH = ROOT / "src/data/preparation/build_manifest.py"
...
class CutClipsHotfixTest(unittest.TestCase):
    def test_source_groups_are_not_limited_to_legacy_tiers(self):
        config(tier="podcast").validate()
        with self.assertRaises(ValueError):
            config(tier="../escape").validate()
