...
// Test multi-choice selection via toggleReasonChoice & confirmReject
S.clips[0].reason='';S.clips[0].dec=undefined;
S.i=0;openReject();
toggleReasonChoice('mouth');toggleReasonChoice('cut');
confirmReject();await Promise.resolve();
assert.equal(requests.at(-1).reason,'mouth, cut');
